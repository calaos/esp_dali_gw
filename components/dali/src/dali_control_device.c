...
esp_err_t dali_103_send_special(dali_master_handle_t handle, uint8_t special_cmd, uint8_t data,
                                bool send_twice, int tx_timeout_ms, int *result)
{
    const uint8_t tx_buf[3] = {DALI_103_SPECIAL_ADDR, special_cmd, data};
    return dali_master_do_raw_transaction(handle, tx_buf, sizeof(tx_buf), send_twice, tx_timeout_ms,
                                          result);
}

esp_err_t dali_103_do_device_command(dali_master_handle_t handle, dali_addr_type_t addr_type,
                                     uint8_t addr, uint8_t command, bool send_twice,
                                     int tx_timeout_ms, int *result)
{
    uint8_t dest;
    switch (addr_type) {
        case DALI_ADDR_SHORT:
            ESP_RETURN_ON_FALSE(addr <= 63, ESP_ERR_INVALID_ARG, TAG,
                                "input device short address out of range: %d", addr);
        ...
        default:
            ESP_LOGE(TAG,
                     "Part 103 device commands currently support short/broadcast addressing only");
...
esp_err_t dali_103_do_instance_command(dali_master_handle_t handle, dali_addr_type_t addr_type,
                                       uint8_t addr, uint8_t instance, uint8_t command,
                                       bool send_twice, int tx_timeout_ms, int *result)
{
    ...
    switch (addr_type) {
        ...
        default:
            ESP_LOGE(
                TAG,
                "Part 103 instance commands currently support short/broadcast addressing only");
            return ESP_ERR_INVALID_ARG;
    }
 ...
 */
static esp_err_t dali_103_set_search_addr_internal(dali_master_handle_t handle, uint32_t addr,
                                                   int tx_timeout_ms)
{
    esp_err_t err;
    err = dali_103_send_special(handle, DALI_103_SPECIAL_SEARCH_ADDR_H,
                                (uint8_t)((addr >> 16) & 0xFFU), false, tx_timeout_ms, NULL);
    if (err != ESP_OK) {
        return err;
    }
    err = dali_103_send_special(handle, DALI_103_SPECIAL_SEARCH_ADDR_M,
                                (uint8_t)((addr >> 8) & 0xFFU), false, tx_timeout_ms, NULL);
    if (err != ESP_OK) {
        return err;
    }
 ...
 */
static esp_err_t dali_103_compare_internal(dali_master_handle_t handle, int tx_timeout_ms,
                                           bool *yes_out)
{
    int result = DALI_RESULT_NO_REPLY;
    esp_err_t err = dali_103_send_special(handle, DALI_103_SPECIAL_COMPARE, 0x00U, false,
                                          tx_timeout_ms, &result);
    if (err != ESP_OK) {
        return err;
    }
    *yes_out = (result == 0xFF);
 ...
 */
static esp_err_t dali_102_send_special_local(dali_master_handle_t handle, uint8_t special_byte,
                                             uint8_t data, bool send_twice, int tx_timeout_ms,
                                             int *result)
{
...
esp_err_t dali_103_commission(dali_master_handle_t handle, dali_commission_mode_t mode,
                              ...
                              int tx_timeout_ms)
{
    ...
    ESP_RETURN_ON_FALSE(max_devices > 0 && max_devices <= 64, ESP_ERR_INVALID_ARG, TAG,
                        "max_devices must be 1-64");
    ...
    ESP_RETURN_ON_ERROR(dali_102_send_special_local(handle, DALI_SPECIAL_TERMINATE, 0x00U, false,
                                                    tx_timeout_ms, NULL),
                        TAG, "[103 commission] Part102 TERMINATE failed");
    vTaskDelay(pdMS_TO_TICKS(50));

    /* Step 1: TERMINATE — abort any ongoing Part 103 commissioning session. */
    ESP_RETURN_ON_ERROR(dali_103_send_special(handle, DALI_103_SPECIAL_TERMINATE, 0x00U, false,
                                              tx_timeout_ms, NULL),
    ...
    ret = dali_103_send_special(handle, DALI_103_SPECIAL_INITIALIZE, init_byte, true, tx_timeout_ms,
                                NULL);
    if (ret != ESP_OK) {
    ...
    ret = dali_102_send_special_local(handle, DALI_SPECIAL_TERMINATE, 0x00U, false, tx_timeout_ms,
                                      NULL);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "[103 commission] Part102 TERMINATE(post-init) failed: %s",
                 esp_err_to_name(ret));
        goto cleanup;
    }
    vTaskDelay(pdMS_TO_TICKS(50));
    ...
    ret =
        dali_103_send_special(handle, DALI_103_SPECIAL_RANDOMIZE, 0x00U, true, tx_timeout_ms, NULL);
    if (ret != ESP_OK) {
    ...
    /* Step 4: Binary-search loop */
    {
        ...
        ESP_LOGI(TAG, "[103 commission] Step4: binary search start (start_addr=%d, max_devices=%d)",
                 start_addr, max_devices);

        while (next_addr < (uint8_t)(start_addr + max_devices)) {
            ret = dali_103_set_search_addr_internal(handle, 0xFFFFFFUL, tx_timeout_ms);
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "[103 commission] SET_SEARCH_ADDR(max) failed: %s",
                         esp_err_to_name(ret));
                goto cleanup;
            }
            bool yes = false;
            ...
            while (low < high) {
                ...
                if (ret != ESP_OK) {
                    ESP_LOGE(TAG, "[103 commission] SET_SEARCH_ADDR(bisect) failed: %s",
                             esp_err_to_name(ret));
                    goto cleanup;
                }
                ret = dali_103_compare_internal(handle, tx_timeout_ms, &yes);
                if (ret != ESP_OK) {
                    ESP_LOGE(TAG, "[103 commission] COMPARE(bisect) failed: %s",
                             esp_err_to_name(ret));
                    goto cleanup;
                }
            ...
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "[103 commission] SET_SEARCH_ADDR(final) failed: %s",
                         esp_err_to_name(ret));
                goto cleanup;
            }
            ...
            ret = dali_103_send_special(handle, DALI_103_SPECIAL_PROGRAM_SHORT_ADDR, encoded, true,
                                        tx_timeout_ms, NULL);
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "[103 commission] PROGRAM_SHORT_ADDR failed: %s",
                         esp_err_to_name(ret));
                goto cleanup;
            }

            ESP_LOGI(TAG, "[103 commission] Assigned short address %d (random=0x%06" PRIX32 ")",
                     next_addr, low);

            ret = dali_103_send_special(handle, DALI_103_SPECIAL_WITHDRAW, 0x00U, false,
                                        tx_timeout_ms, NULL);
            if (ret != ESP_OK) {
...
cleanup:
     ...
     * preserved in ret. */
    {
        esp_err_t term_err = dali_103_send_special(handle, DALI_103_SPECIAL_TERMINATE, 0x00U, false,
                                                   tx_timeout_ms, NULL);
        if (term_err != ESP_OK) {
            ESP_LOGW(TAG, "[103 commission] TERMINATE(final) failed: %s",
                     esp_err_to_name(term_err));
        }
    }
    ...
    ESP_RETURN_ON_ERROR(dali_103_do_device_command(handle, DALI_ADDR_BROADCAST, 0,
                                                   DALI_103_START_QUIESCENT_MODE, true,
                                                   tx_timeout_ms, NULL),
                        TAG, "[103 commission] START_QUIESCENT_MODE failed");
    vTaskDelay(pdMS_TO_TICKS(50));

    ESP_LOGI(TAG, "[103 commission] Complete: %d input device(s) addressed (start_addr=%d)", found,
             start_addr);
...
esp_err_t dali_103_query_device_status(dali_master_handle_t handle, dali_addr_type_t addr_type,
                                       uint8_t addr, uint8_t *status, int tx_timeout_ms)
{
    ESP_RETURN_ON_FALSE(handle != NULL && status != NULL, ESP_ERR_INVALID_ARG, TAG,
                        "handle and status must not be NULL");

    int result = DALI_RESULT_NO_REPLY;
    ESP_RETURN_ON_ERROR(dali_103_do_device_command(handle, addr_type, addr,
                                                   DALI_103_QUERY_DEVICE_STATUS, false,
                                                   tx_timeout_ms, &result),
...
esp_err_t dali_103_reset_device(dali_master_handle_t handle, dali_addr_type_t addr_type,
                                uint8_t addr, int tx_timeout_ms)
{
    ESP_RETURN_ON_FALSE(handle != NULL, ESP_ERR_INVALID_ARG, TAG, "handle must not be NULL");
    return dali_103_do_device_command(handle, addr_type, addr, DALI_103_RESET, true, tx_timeout_ms,
                                      NULL);
}

esp_err_t dali_103_query_instance_type(dali_master_handle_t handle, dali_addr_type_t addr_type,
                                       uint8_t addr, uint8_t instance, uint8_t *type,
                                       int tx_timeout_ms)
{
    ESP_RETURN_ON_FALSE(handle != NULL && type != NULL, ESP_ERR_INVALID_ARG, TAG,
                        "handle and type must not be NULL");

    int result = DALI_RESULT_NO_REPLY;
    ESP_RETURN_ON_ERROR(dali_103_do_instance_command(handle, addr_type, addr, instance,
                                                     DALI_103_QUERY_INSTANCE_TYPE, false,
                                                     tx_timeout_ms, &result),
...
esp_err_t dali_103_query_number_of_instances(dali_master_handle_t handle,
                                             dali_addr_type_t addr_type, uint8_t addr,
                                             uint8_t *num_instances, int tx_timeout_ms)
{
    ...
    int result = DALI_RESULT_NO_REPLY;
    ESP_RETURN_ON_ERROR(dali_103_do_device_command(handle, addr_type, addr,
                                                   DALI_103_QUERY_NUMBER_OF_INSTANCES, false,
                                                   tx_timeout_ms, &result),
                        TAG, "QUERY_NUMBER_OF_INSTANCES failed");
