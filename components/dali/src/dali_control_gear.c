 ...
 */
static esp_err_t dali_102_send_special(dali_master_handle_t handle, uint8_t special_byte,
 ...
 */
static esp_err_t dali_102_set_search_addr(dali_master_handle_t handle, uint32_t addr,
                                          int tx_timeout_ms)
{
    esp_err_t err;
    ...
    return dali_102_send_special(handle, DALI_SPECIAL_SEARCH_ADDR_L, (uint8_t)(addr & 0xFFU), false,
                                 tx_timeout_ms, NULL);
}
...
static esp_err_t dali_102_compare(dali_master_handle_t handle, int tx_timeout_ms, bool *yes_out)
{
    ...
    esp_err_t err =
        dali_102_send_special(handle, DALI_SPECIAL_COMPARE, 0x00U, false, tx_timeout_ms, &result);
    if (err != ESP_OK) {
        return err;
    }
    *yes_out = (result == 0xFF);
                          ...
                          int tx_timeout_ms)
{
    ...
    ESP_RETURN_ON_ERROR(dali_103_do_device_command(handle, DALI_ADDR_BROADCAST, 0,
                                                   ...
                                                   tx_timeout_ms, NULL),
                        TAG, "[102 commission] Part103 START_QUIESCENT_MODE failed");
    ...
    ESP_RETURN_ON_ERROR(
        dali_102_send_special(handle, DALI_SPECIAL_TERMINATE, 0x00U, false, tx_timeout_ms, NULL),
        TAG, "[102 commission] TERMINATE failed");
    ...
    ESP_RETURN_ON_ERROR(dali_102_send_special(handle, DALI_SPECIAL_INITIALIZE, init_byte, true,
                                              tx_timeout_ms, NULL),
    ...
    ESP_RETURN_ON_ERROR(
        dali_102_send_special(handle, DALI_SPECIAL_RANDOMIZE, 0x00U, true, tx_timeout_ms, NULL),
        TAG, "[102 commission] RANDOMISE failed");
    ...
    ESP_LOGI(TAG, "[102 commission] Step4: binary search start (start_addr=%d, max_devices=%d)",
    ...
    while (next_addr < (uint8_t)(start_addr + max_devices)) {
        ...
        ESP_RETURN_ON_ERROR(dali_102_compare(handle, tx_timeout_ms, &yes), TAG,
                            "[102 commission] COMPARE(max) failed");
        ...
        ESP_RETURN_ON_ERROR(dali_102_send_special(handle, DALI_SPECIAL_PROGRAM_SHORT_ADDR, encoded,
                                                  true, tx_timeout_ms, NULL),
                            TAG, "[102 commission] PROGRAM_SHORT_ADDR failed");

        ESP_LOGI(TAG, "[102 commission] Assigned short address %d (random=0x%06" PRIX32 ")",
                 next_addr, low);

        ESP_RETURN_ON_ERROR(
            dali_102_send_special(handle, DALI_SPECIAL_WITHDRAW, 0x00U, true, tx_timeout_ms, NULL),
    ...
    ESP_LOGI(TAG, "[102 commission] Complete: %d gear addressed (start_addr=%d)", found,
             start_addr);
