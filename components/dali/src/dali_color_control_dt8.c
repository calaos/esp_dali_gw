 ...
 */
static esp_err_t dali_set_dtr(dali_master_handle_t handle, uint8_t dtr_addr_byte, uint8_t value,
                              int tx_timeout_ms)
{
...
esp_err_t dali_enable_device_type(dali_master_handle_t handle, uint8_t device_type,
                                  int tx_timeout_ms)
{
    dali_master_transaction_config_t cfg = {
        .addr_type = DALI_ADDR_SPECIAL,
...
esp_err_t dali_master_set_color(dali_master_handle_t handle, dali_addr_type_t addr_type,
                                uint8_t addr, dali_color_mode_t mode, dali_color_val_t val,
                                int tx_timeout_ms)
{
    ...
    if (mode == DALI_COLOR_CCT) {
        ...
        ESP_LOGD(TAG, "Part 209 CCT: addr_type=%d addr=%u mirek=%u", addr_type, addr,
                 val.cct.mirek);

    } else if (mode == DALI_COLOR_RGB) {
        ESP_RETURN_ON_ERROR(
            dali_set_dtr(handle, DALI_SPECIAL_DATA_TRANSFER_REG, val.rgb.r, tx_timeout_ms), TAG,
            "SET DTR0 (R) failed");
        ESP_RETURN_ON_ERROR(
            dali_set_dtr(handle, DALI_SPECIAL_DATA_TRANSFER_REG1, val.rgb.g, tx_timeout_ms), TAG,
            "SET DTR1 (G) failed");
        ESP_RETURN_ON_ERROR(
            dali_set_dtr(handle, DALI_SPECIAL_DATA_TRANSFER_REG2, val.rgb.b, tx_timeout_ms), TAG,
            "SET DTR2 (B) failed");
        color_cmd = DALI_209_SET_TEMPORARY_RGB_DIMLEVEL;
        ESP_LOGD(TAG, "Part 209 RGB: addr_type=%d addr=%u r=%u g=%u b=%u", addr_type, addr,
                 val.rgb.r, val.rgb.g, val.rgb.b);

    } else if (mode == DALI_COLOR_RGBWAF) {
        /* Step 1: Load R/G/B; send SET_TEMPORARY_RGB_DIMLEVEL. */
        ESP_RETURN_ON_ERROR(
            dali_set_dtr(handle, DALI_SPECIAL_DATA_TRANSFER_REG, val.rgbwaf.r, tx_timeout_ms), TAG,
            "SET DTR0 (R) failed");
        ESP_RETURN_ON_ERROR(
            dali_set_dtr(handle, DALI_SPECIAL_DATA_TRANSFER_REG1, val.rgbwaf.g, tx_timeout_ms), TAG,
            "SET DTR1 (G) failed");
        ESP_RETURN_ON_ERROR(
            dali_set_dtr(handle, DALI_SPECIAL_DATA_TRANSFER_REG2, val.rgbwaf.b, tx_timeout_ms), TAG,
            "SET DTR2 (B) failed");
        ...
        ESP_RETURN_ON_ERROR(dali_enable_device_type(handle, 8, tx_timeout_ms), TAG,
                            "ENABLE_DEVICE_TYPE 8 before RGB failed");
        ESP_RETURN_ON_ERROR(dali_master_do_transaction(handle, &rgb_cfg, NULL), TAG,
                            "SET_TEMPORARY_RGB_DIMLEVEL failed");

        /* Step 2: Load W/A/F; fall through to send SET_TEMPORARY_WAF_DIMLEVEL below. */
        ESP_RETURN_ON_ERROR(
            dali_set_dtr(handle, DALI_SPECIAL_DATA_TRANSFER_REG, val.rgbwaf.w, tx_timeout_ms), TAG,
            "SET DTR0 (W) failed");
        ESP_RETURN_ON_ERROR(
            dali_set_dtr(handle, DALI_SPECIAL_DATA_TRANSFER_REG1, val.rgbwaf.a, tx_timeout_ms), TAG,
            "SET DTR1 (A) failed");
        ESP_RETURN_ON_ERROR(
            dali_set_dtr(handle, DALI_SPECIAL_DATA_TRANSFER_REG2, val.rgbwaf.f, tx_timeout_ms), TAG,
            "SET DTR2 (F) failed");

        color_cmd = DALI_209_SET_TEMPORARY_WAF_DIMLEVEL;
        ESP_LOGD(TAG, "Part 209 RGBWAF: addr_type=%d addr=%u r=%u g=%u b=%u w=%u a=%u f=%u",
                 addr_type, addr, val.rgbwaf.r, val.rgbwaf.g, val.rgbwaf.b, val.rgbwaf.w,
                 val.rgbwaf.a, val.rgbwaf.f);
